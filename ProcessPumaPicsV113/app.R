...
ui <- fluidPage(
  ...
  sidebarLayout(
    ...
    sidebarPanel(
      ...
      ######F. Overwrite files?######
      tags$p("6 - Do you wish to overwrite files? "),
      #Yes/No as the whether or not to overwrite files if they already exist in folder.
      #Likely impertenent and can be removed in future releases
      #Default is YES
      radioButtons(inputId = "ovr.write", 
                   label = NULL,
                   choices = c("Yes" = TRUE, "No" = FALSE), selected = TRUE, inline = T),
...
server = function(input, output, session) {
  ...
  observeEvent(input$run, {
    ...
    overwrite.tf <- as.logical(input$ovr.write)
